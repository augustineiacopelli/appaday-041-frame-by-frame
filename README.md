...
- **Drag to reposition** — tap the crosshair button in the controls bar to enter reposition mode, then drag the photo within the frame to set the crop point per image. Position is saved individually per photo and persists across sessions
...
- Swipe left/right to advance manually on touch devices (disabled during reposition mode)
...
- Photos, frame choice, interval, and per-photo crop positions persist across sessions via localStorage
